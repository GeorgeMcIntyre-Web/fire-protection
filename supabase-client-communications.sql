...
INSERT INTO client_communications (project_id, client_name, message, sent_by, communication_type)
SELECT 
  p.id,
  c.name,
  'Project is progressing well. Currently at ' || 
  CASE 
    WHEN p.status = 'completed' THEN '100% completion.'
    WHEN p.status = 'in_progress' THEN '50% completion. Next phase starting soon.'
    ELSE 'initial stage. Planning and design phase.'
  END,
  p.created_by,
  'update'
FROM projects p
